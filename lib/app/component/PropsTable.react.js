...
var PropsTable = (0, _radium2.default)(_class = (_temp = _class2 = function (_Component) {
  ...
  _createClass(PropsTable, [{
    ...
    value: function render() {
      ...
      if (Object.keys(propsDefinition).length === 0) return _react2.default.createElement(
        'div',
        { style: styles.prop.noProps },
        'No props defined'
      );

      var sortedProps = Object.keys(propsDefinition).sort().reduce(function (acc, k) {
        return acc.set(k, propsDefinition[k]);
      ...
      return _react2.default.createElement(
        ...
        sortedProps.map(function (value, key) {
          return _this2.renderProp(value, key, true);
        }),
        sortedProps.map(function (value, key) {
          return _this2.renderProp(value, key, false);
        })
      );
    }
  }, {
    key: 'renderProp',
    value: function renderProp(data, key, renderRequired) {
      if (!data.type) return null;

      var required = data.required;

      if (renderRequired && !required || !renderRequired && required) return null;
    ...
    value: function renderPropTableRow(data, key, renderRequired, scope) {
      ...
      var activeProps = _props.activeProps;
      var commonStyles = _props.commonStyles;
      var triggeredProps = _props.triggeredProps;


      if (!data.type) return null;

      if (data.type.name === 'shape') return (0, _immutable.Map)(data.type.value).map(function (v, k) {
        return _this3.renderPropTableRow({ type: v }, k, renderRequired, [key]);
      });

      var required = data.required;
      var triggered = triggeredProps.includes(key);
      var fullWidth = ['any', 'array', 'arrayOf', 'element', 'enum', 'node', 'object', 'shape', 'string'].indexOf(data.type.name) !== -1;
      return _react2.default.createElement(
        'div',
        { key: key },
        _react2.default.createElement(
          ...
          _react2.default.createElement(
            ...
            data.type.name === 'func' ? 'func()' : this.renderValueSelection(key, data.type, scope, data)
          ),
        ...
        _react2.default.createElement('div', { styles: styles.clearfix })
      );
    }
  }, {
    ...
    value: function renderValueSelection(key, type) {
      ...
      var defaultProps = {
        ...
        value: /*(data.bluebird && data.bluebird.defaultVal) ||*/(0, _immutable.fromJS)(_extends({}, componentProps)).getIn(scope.concat([key]))
      };
    ...
    value: function selectOptions(type) {
      ...
      if (!type.required) return [_react2.default.createElement('option', { value: '' })].concat(options);
...
var styles = {
  ...
  prop: {
    ...
    description: _extends({}, _Font2.default.size.small, {
      borderLeft: '5px solid transparent',
      padding: '0 ' + _Spaces2.default.small + ' 0 ' + _Spaces2.default.smaller,
      transition: 'all .2s ease-out',
      margin: '5px 0 15px 0',
